...
```mermaid
flowchart TD
    A[1. Học viên nhận yêu cầu làm lab] --> B[2. Đọc slide / worksheet / ví dụ mẫu]
    B --> C[3. Đọc thêm thông báo trong chat hoặc lời nhắc mentor]
    C --> D[4. Tự ghép lại thứ tự làm bài]
    D --> E{Có chắc yêu cầu đúng chưa?}
    E -->|Không chắc| F[5. Hỏi bạn cùng nhóm hoặc mentor/TA]
    F --> G[6. Chờ phản hồi / tự đối chiếu lại]
    G --> H[7. Bắt đầu làm bài cá nhân hoặc nhóm]
    E -->|Tạm chắc| H
    H --> I[8. Trước khi nộp, kiểm tra lại nhiều nguồn]
    I --> J{Bài đã đủ yêu cầu chưa?}
    J -->|Chưa chắc| F
    J -->|Tạm đủ| K[9. Nộp bài]
...
```mermaid
flowchart TD
    A[1. Học viên mở source of truth duy nhất] --> B[2. Rule hiển thị luồng làm bài theo phase]
    B --> C[3. Rule hiển thị checklist bắt buộc]
    C --> D{Học viên có câu hỏi không?}

    D -->|Không| E[4. Làm bài theo checklist]
    D -->|Có| F[5. AI/Workflow assistant tìm câu trả lời trong tài liệu đã duyệt]
    F --> G{Có nguồn rõ ràng không?}

    G -->|Có| H[6. AI trả lời kèm link/source liên quan]
    G -->|Không hoặc mâu thuẫn| I[7. Chuyển mentor/TA xác nhận]

    H --> E
    I --> E

    E --> J[8. Học viên self-check bài nháp]
    J --> K[9. AI/checklist chỉ ra phần thiếu hoặc mơ hồ]
    K --> L{Bài đã đủ checklist chưa?}

    L -->|Chưa| M[10. Học viên bổ sung phần thiếu]
    M --> J
    L -->|Đủ| N[11. Nộp bài]
    ...
    classDef rule fill:#f0f0f0,stroke:#555,stroke-width:1px;
